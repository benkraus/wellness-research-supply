    ...
    <li className="flex px-4 py-6 sm:px-6">
      {thumbnail && (
        <div className="flex-shrink-0">
          <Image src={thumbnail} alt={item.product_title || 'product thumbnail'} className="w-20 rounded-md" />
        </div>
      )}

      <div className={clsx('flex flex-1 flex-col', thumbnail && 'ml-6')}>
        <div className="flex">
          <div className="min-w-0 flex-1">
            <h4 className="text-base">
              <Link
                to={`/products/${item.product_handle}`}
                className="font-bold text-primary-50 hover:text-primary-100"
              >
                {item.product_title}
              </Link>
            </h4>
...
export const CheckoutOrderSummaryItems: FC<CheckoutOrderSummaryItemsProps> = ({ cart, name }) => (
  <ul className="divide-y divide-primary-900/40">
    {cart.items?.map((item) => (
