...
export const loader = async ({ request, params }: LoaderFunctionArgs) => {
	...
	const [order, batchAllocations, tracking] = await Promise.all([
		retrieveOrder(request, orderId).catch(() => null),
		listOrderVariantBatches(request, orderId).catch(() => null),
		listOrdersTracking(request, [orderId]).catch(() => ({ orders: [] })),
	]);
	if (!order) {
		throw redirect("/account");
	}

	const trackingEntry = tracking.orders.find(
	...
	return {
		order,
		batchAllocations,
		trackingPackages: trackingEntry?.packages ?? [],
	} as {
		order: StoreOrder;
		batchAllocations: typeof batchAllocations;
...
export default function AccountOrderDetailRoute() {
	const { order, batchAllocations, trackingPackages } =
		useLoaderData<typeof loader>();
	const batchMap = new Map(
		(batchAllocations?.items ?? []).map((item) => [
			item.line_item_id,
			item.batches,
		]),
	);
							...
							<div className="mt-4 space-y-4">
								{order.items?.map((item) => {
									...
									return (
										<div
											key={item.id}
											className="border-b border-primary-900/10 pb-4"
										>
											<div className="flex flex-wrap items-start justify-between gap-4">
												<div>
											...
											<div className="mt-3">
												...
												{batches.length === 0 ? (
													<p className="mt-2 text-xs text-primary-200">
														Batch details will appear once available.
													</p>
												) : (
								...
								<div className="mt-4 space-y-3">
									{trackingPackages.map((pkg) => (
										<div
											...
											className="rounded-lg border border-primary-900/10 bg-highlight-100 p-4 text-sm text-primary-200"
										>
											<div className="flex flex-wrap items-center justify-between gap-3">
												<div className="space-y-1">
													...
													<p className="text-xs uppercase tracking-[0.2em] text-primary-300">
														{formatTrackingStatus(pkg.tracking_status)}
													</p>
												</div>
												<div className="text-right">
													...
													<p className="text-primary-50">
														{pkg.shipped_at
															? formatDate(new Date(pkg.shipped_at))
															: "—"}
													</p>
												</div>
											</div>
											{pkg.tracking_url && (
												<a
													...
													target="_blank"
													rel="noreferrer"
													className="mt-3 inline-flex text-sm font-semibold text-primary-50 underline decoration-primary-200/40 underline-offset-4"
												>
													Track package
												</a>
											)}
